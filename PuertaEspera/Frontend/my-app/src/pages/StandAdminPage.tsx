    ...
    const handleDescargarQR = () => {
        ...
        img.onload = () => {
            ...
            img.onload = () => {
                ...
                if (ctx) {
                    // 2. ¡IMPORTANTE! NO pintamos el fondo.
                    // Al no usar fillRect con blanco, el canvas nace transparente.
                    // Simplemente limpiamos por seguridad.
                    ctx.clearRect(0, 0, canvas.width, canvas.height);

                    // 3. Dibujamos el QR (que vendrá en BLANCO desde el componente)
                    ctx.drawImage(img, margin, margin);

                    // 4. Generamos el PNG (que soporta transparencia)
                    const pngFile = canvas.toDataURL("image/png");

                    const downloadLink = document.createElement("a");
                    // Le ponemos un nombre claro para tu compañera
            ...
            <div className={`min-h-screen ${bgClass} px-6 py-8 flex flex-col items-center relative overflow-x-hidden transition-colors duration-500`}>
                ...
                {showQRModal && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-200">
                        ...
                        {/* Tarjeta oscura para poder ver el QR blanco */}
                        <div className="bg-gray-900 border border-gray-700 rounded-3xl p-8 w-full max-w-sm flex flex-col items-center shadow-2xl relative animate-in zoom-in duration-300">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2 uppercase tracking-widest font-dm-sans">
                                Asset para Diseño
                            </h3>
                            <p className="text-xs text-gray-400 mb-8 text-center px-4">
                                Este QR es blanco y con fondo transparente.
                            </p>

                            {/* CONTENEDOR DEL QR 
                            Le pongo un fondo temporal gris oscuro solo para que VOS lo veas en pantalla.
                            Ese fondo NO se descargará.
                        */}
                            <div className="p-4 rounded-xl border-2 border-dashed border-gray-600 bg-transparent mb-8">
                                <QRCode
                                    id="StandQR"
                                    value={qrTargetUrl}
                                    size={256}
                                    level="H"
                                    fgColor="#FFFFFF" // <--- PUNTOS BLANCOS (Lo que quiere la diseñadora)
                            ...
                            <button
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 uppercase tracking-wide text-sm"
                            >
